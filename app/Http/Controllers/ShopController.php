...
class ShopController extends Controller
{
    ...
    {
        $order = OrderModel::find($id_order)->load([
            'bill' => function ($bill) {
                $bill->with('address_receive');
            }
        ]);
        $user_model = User::find($order->id_user);
        return Response()->json([
            'address_receive' => $order->bill->address_receive,
            'user_model' => $user_model,
            'state_payment' => $order->bill->is_payment
        ]);
    }
